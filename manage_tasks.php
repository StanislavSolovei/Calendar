...
if (isset($_GET['get_form'])) {
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        echo "<button type='submit' name='update_tasks'>Zmienić wszystko</button>";
        echo "</form><hr>";
    }
...
if (isset($_POST['add_task'])) {
    ...
    mysqli_query($conn, "INSERT INTO zadania (user_id, data_zadania, wpis) VALUES ('$user_id', '$date', '$text')");
...
if (isset($_POST['update_tasks'])) {
    foreach ($_POST['tasks'] as $id => $text) {
        ...
        mysqli_query($conn, "UPDATE zadania SET wpis='$text' WHERE id='$id' AND user_id='$user_id'");
    }
